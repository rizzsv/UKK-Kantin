...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🍔 Fetching food menu with search:", search);
    ...
    const response = await fetch(
      "https://ukk-p2.smktelkom-mlg.sch.id/api/getmenufood",
      {
        method: "POST",
        headers: {
          Authorization: token || "",
    ...
    console.log("✅ Food menu response status:", response.status);
    console.log("✅ Food menu data structure:", {
      hasData: !!data,
    ...
    if (items.length > 0) {
      console.log("✅ First food item:", JSON.stringify(items[0], null, 2));
    }

    return NextResponse.json(data, { status: response.status });
  } catch (error) {
    console.error("❌ Food menu proxy error:", error);
    return NextResponse.json(
      {
